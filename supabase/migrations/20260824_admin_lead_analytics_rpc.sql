-- ============================================================================
-- Lead-Analyse Phase 1 — Schritt 5/5: Admin-Auswertungen
-- ============================================================================
-- Alle Funktionen sind SECURITY DEFINER und werden ausschliesslich von der
-- service_role über api/admin-lead-analytics.js aufgerufen. EXECUTE ist für
-- anon und authenticated explizit entzogen — die Auswertungen dürfen nie
-- direkt aus dem Browser erreichbar sein.
--
-- Die Aggregation läuft bewusst in der Datenbank: Der Anwendungscode müsste
-- sonst pro Anbieter eine Leadabfrage stellen (N+1) oder alle Leads in den
-- Serverspeicher laden.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Anbieterübersicht mit Leadstatistiken
-- ----------------------------------------------------------------------------
-- Such-, Filter-, Sortier- und Seitenparameter werden hier hart validiert; die
-- API prüft zusätzlich vor. p_sort/p_dir werden über CASE-Ausdrücke aufgelöst
-- statt über dynamisches SQL — damit ist keine SQL-Injection über den
-- Sortierparameter möglich.
...
BEGIN
  ...
  prev_period AS (
    -- Zuletzt beendete Periode = vorheriges Paket.
  ...
  qualified AS (
    -- Nur die aktuelle Basic-Phase; identische Bedingungen wie in
    -- recompute_basic_lead_ranking_factors().
  ...
  filtered AS (
    ...
    WHERE v_filter IS NULL
       ...
       -- "Basic mit vielen Leads": ab der ersten Abstufung der Staffel.
  ...
  ORDER BY
    CASE WHEN v_asc THEN
      CASE v_sort
        WHEN 'leads_total'   THEN c.leads_total
        WHEN 'leads_30d'     THEN c.leads_30d
        WHEN 'leads_90d'     THEN c.leads_90d
        WHEN 'leads_365d'    THEN c.leads_365d
        WHEN 'qualified_basic' THEN c.qualified_basic
      END
    END ASC NULLS LAST,
    ...
    -- Stabiler Endanker, damit die Seitenaufteilung reproduzierbar bleibt.
    c.id ASC
...
-- ----------------------------------------------------------------------------
-- Kennzahlen eines einzelnen Anbieters
-- ----------------------------------------------------------------------------
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'totals', (
      SELECT jsonb_build_object(
        'leads_30d',  count(*) FILTER (WHERE l.created_at >= now() - INTERVAL '30 days'),
        'leads_90d',  count(*) FILTER (WHERE l.created_at >= now() - INTERVAL '90 days'),
        'leads_365d', count(*) FILTER (WHERE l.created_at >= now() - INTERVAL '365 days'),
        'leads_total', count(*),
        'avg_quality_30d',  avg(l.quality_score) FILTER (WHERE l.quality_score IS NOT NULL AND l.created_at >= now() - INTERVAL '30 days'),
        'avg_quality_90d',  avg(l.quality_score) FILTER (WHERE l.quality_score IS NOT NULL AND l.created_at >= now() - INTERVAL '90 days'),
        'avg_quality_365d', avg(l.quality_score) FILTER (WHERE l.quality_score IS NOT NULL AND l.created_at >= now() - INTERVAL '365 days'),
        'avg_quality_total', avg(l.quality_score) FILTER (WHERE l.quality_score IS NOT NULL),
        'scored_total', count(*) FILTER (WHERE l.quality_score IS NOT NULL),
        ...
        -- Aufteilung nach der Paketphase, in der der Lead ankam.
        'leads_during_basic', count(*) FILTER (WHERE l.provider_tier_at_lead = 'basic'),
        'leads_during_paid', count(*) FILTER (WHERE l.provider_tier_at_lead IN ('pro', 'premium', 'enterprise')),
        'leads_tier_unknown', count(*) FILTER (WHERE l.provider_tier_at_lead IS NULL),
        'qualified_basic_current_phase', count(*) FILTER (
          WHERE v_basic_start IS NOT NULL
            AND l.status = 'sent'
            AND l.provider_tier_at_lead = 'basic'
            AND l.quality_score >= 5
            AND l.created_at >= v_basic_start
        )
      )
      FROM leads l WHERE l.provider_id = p_provider_id
    ),
    -- Monatsverlauf über den gesamten vorhandenen Zeitraum, nicht nur 12 Monate.
    'monthly', coalesce((
    ...
    'by_course', coalesce((
      SELECT jsonb_agg(c ORDER BY (c->>'leads')::INT DESC)
      FROM (
        SELECT jsonb_build_object(
          'course_id', l.course_id,
          -- NULL, wenn der Kurs gelöscht wurde. Der Lead bleibt trotzdem in
          -- der Statistik (ON DELETE SET NULL).
          'title', co.title,
          'leads', count(*),
          'avg_quality', avg(l.quality_score) FILTER (WHERE l.quality_score IS NOT NULL)
        ) AS c
    ...
    'score_distribution', coalesce((
      SELECT jsonb_object_agg(sd.score::TEXT, sd.cnt)
      FROM (
        SELECT l.quality_score AS score, count(*) AS cnt
        FROM leads l
        WHERE l.provider_id = p_provider_id AND l.quality_score IS NOT NULL
    ...
    'package_history', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        'id', h.id,
        'package_tier', h.package_tier,
        'started_at', h.started_at,
        'ended_at', h.ended_at,
        'start_is_estimated', h.start_is_estimated,
...
-- ----------------------------------------------------------------------------
-- Einzelne Leads eines Anbieters (Liste, ohne Klartext)
-- ----------------------------------------------------------------------------
-- Liefert bewusst NUR die Metadaten plus die Information, ob der Anfragetext
-- noch existiert. Der Klartext wird ausschliesslich beim gezielten Öffnen eines
-- einzelnen Leads entschlüsselt (api/admin-lead-analytics.js, action=message).
CREATE OR REPLACE FUNCTION admin_provider_leads(
  p_provider_id UUID,
...
-- ----------------------------------------------------------------------------
-- Rechte: ausschliesslich service_role
-- ----------------------------------------------------------------------------
...
REVOKE ALL ON FUNCTION recompute_basic_lead_ranking_factors() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION cleanup_expired_lead_messages() FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION recompute_basic_lead_ranking_factors() TO service_role;
GRANT EXECUTE ON FUNCTION cleanup_expired_lead_messages() TO service_role;
