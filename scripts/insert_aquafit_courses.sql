-- ============================================================================
-- Kurse für AquaFit4You / AquaTime
...
--
-- HINWEIS:
-- Da alle Kurse an drei Standorten angeboten werden (Zürich, St. Gallen, Obwalden),
-- wird pro Kurs ein Haupt-Eintrag erstellt. Die Standorte werden dann über
-- course_events für spezifische Termine hinzugefügt.
--
-- Taxonomie-Kategorien (bereits in DB vorhanden):
-- - privat_hobby > sport_fitness > Wassersport ✓
-- - kinder_jugend > freizeit_hobbys > Kinderturnen & Sport ✓
-- ============================================================================
...
INSERT INTO courses (
    ...
    'privat_hobby',
    'sport_fitness',
    'Wassersport', -- BITTE PRÜFEN: Muss in taxonomy_specialties existieren
    'Aqua-Mama (Schwangerschaftsfitness)',
    6,
    '45 - 60 Minuten pro Lektion (fortlaufendes Abo)',
    'https://aquafit4you.ch/aqua-mama/',
    'lead',
    'draft', -- Als Draft erstellen, dann vom Anbieter prüfen lassen
    false,
    NULL, -- ERGÄNZEN: z.B. 'Zürich', 'St. Gallen', oder 'Obwalden'
    NULL  -- ERGÄNZEN: z.B. 'Zürich' (nur Stadt, keine Strasse bei Lead-Booking)
) RETURNING id;

-- TODO: Nach dem Insert die course_id notieren und in course_private ergänzen:
-- INSERT INTO course_private (course_id, contact_email)
-- VALUES ('[COURSE_ID]', '[KONTAKT_EMAIL]');
...
INSERT INTO courses (
    ...
    'Die Kinderschwimmkurse von AquaTime basieren auf dem bewährten Schweizer Ausbildungssystem von Swimsports.ch und begleiten Kinder spielerisch von der ersten Wassergewöhnung bis hin zum sicheren Schwimmen. In den sieben Grundlagentests – beginnend beim "Krebs" für die Jüngsten bis hin zum "Eisbär" für fortgeschrittene Schwimmer – werden die Kinder stufengerecht gefördert. Im Fokus stehen zunächst das Vertrauen zum Element Wasser, das Tauchen, das Atmen und das Schweben. Mit zunehmendem Niveau werden die technischen Grundlagen der verschiedenen Schwimmstile wie Crawl, Rückencrawl und Brustschwimmen sowie die koordinativen Fähigkeiten systematisch aufgebaut. Der Unterricht findet in Kleingruppen statt, was eine intensive Betreuung und eine hohe Sicherheit gewährleistet. Jede Stufe schliesst mit einem offiziellen Abzeichen ab, das den Lernfortschritt dokumentiert und die Motivation der Kinder steigert. Ziel des Kurses ist es, den Kindern eine solide technische Basis zu vermitteln, damit sie sich lebenslang sicher und mit Freude im Wasser bewegen können. Die spielerische Vermittlung sorgt dafür, dass die Freude an der Bewegung stets im Vordergrund steht, während gleichzeitig wichtige Sicherheitsregeln verinnerlicht werden.',
    ...
    'kinder_jugend',
    'sport_fitness',
    'Schwimmschule', -- BITTE PRÜFEN: Muss in taxonomy_specialties existieren
    'Grundlagentests (Technik)',
    10, -- Ca. 10-12 Lektionen pro Quartal
    '30 - 45 Minuten pro Lektion',
    'https://aquatime.ch/kinderschwimmen/',
    'lead',
    'draft',
    false,
    NULL, -- ERGÄNZEN: Kanton
    NULL  -- ERGÄNZEN: Stadt
) RETURNING id;
...
INSERT INTO courses (
    ...
    'Schwimmschule',
    'Erwachsenentraining',
    NULL, -- Blockkurse oder fortlaufend
...
--
--    INSERT INTO course_private (course_id, contact_email)
--    VALUES ('[COURSE_ID_FROM_ABOVE]', 'kontakt@aquafit4you.ch');
--
-- 2. Standorte/Kantone ergänzen:
--    Entweder direkt in den INSERT-Statements oben die Felder `canton` und
--    `address` befüllen, oder nach dem Insert aktualisieren:
--
--    UPDATE courses
--    SET canton = 'Zürich', address = 'Zürich'
--    WHERE id = '[COURSE_ID]';
--
-- 3. Taxonomie prüfen:
--    Die verwendeten Kategorien müssen in der Taxonomie existieren:
--    - 'Wassersport' als specialty unter sport_fitness
--    - 'Schwimmschule' als specialty unter sport_fitness
--
--    Prüfen mit:
--    SELECT * FROM taxonomy_specialties
--    WHERE area_id IN (
--        SELECT id FROM taxonomy_areas WHERE slug = 'sport_fitness'
--    );
--
-- 4. Optional: Bilder hochladen
