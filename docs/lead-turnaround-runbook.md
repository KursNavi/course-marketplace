...
`landing_view` → `search_view` → `course_card_cta_click` oder `course_detail_view` → `lead_form_start` → `lead_submitted` → `lead_delivered` → `booking_start` → `booking_completed`
...
Die Freigabe wird wöchentlich dokumentiert: Datum, Topic, Region, Kurszahl, Anbieterzahl, Vollständigkeit, zugestellte Leads, 30-Tage-Leads, CPL und Entscheidung.
...
7. Budget erst erhöhen, wenn die Zelle mindestens 15 gültige Leads und einen CPL von höchstens CHF 75 erreicht.
...
## 5. Anbieter-Kommunikation

- Direkt nach dem Lead: strukturierte Anbieter-E-Mail mit den Kontaktdaten und einem klaren Hinweis, direkt auf die E-Mail zu antworten.
- Es gibt keine automatische Statuspflege, Erinnerungs- oder Eskalations-E-Mails für Anbieter.
- Operations kann bei konkreten Supportfällen manuell unterstützen und der anfragenden Person Alternativen anbieten.
...
| wöchentlich | Zugestellte Leads und Rückmeldungen im Support | priorisierte Supportfälle |
| alle 2 Wochen | genau ein Conversion-Experiment | Gewinner/Verlierer mit Entscheidung |
| monatlich | CPL, Leadqualität und Zustellquote je Zelle | Budget halten, erhöhen oder pausieren |
...
- Testanfrage ohne Nachricht ausführen; Datensatz, Anbieterzustellung, Bestätigungs-E-Mail und Referenz prüfen.
- Resend-Webhook mit `email.delivered` und einem Bounce-Fall prüfen; nur der Zustellfall darf die primäre Conversion auslösen. Anbieterstatus und statusbasierte Cron-Erinnerungen sind nicht Teil des Ablaufs.
...
| Tag 60 | Zustellquote und Leadqualität nach Kanal, Thema und Region auswertbar |
| Tag 90 | Paid Click-to-valid-Lead mindestens 2,0 %, CPL höchstens CHF 75 |
| Tag 90 | Leadqualität und Nachfrage je Zelle verbessert; jede Buchung genau einmal erfasst |
